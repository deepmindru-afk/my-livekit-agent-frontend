...
export const Welcome = React.forwardRef<HTMLDivElement, React.ComponentProps<'div'> & WelcomeProps>(({
  ...
  const stats = [
    ...
    { value: 'Realistic', label: 'Human-Friendly Avatar', sublabel: 'Engages users with lifelike interaction' },
    ...
    {/* 🌙 Transparent & Aesthetic Scrollbar Styling */}
      ...
      <div className="absolute inset-0 -z-10 overflow-hidden pointer-events-none">
        <div className="absolute inset-0">
          <div className="absolute w-64 h-64 sm:w-96 sm:h-96 bg-gradient-to-r from-purple-500/5 to-violet-500/5 rounded-full blur-3xl animate-float -z-10" style={{ top: '-10%', left: '-5%' }} />
          <div className="absolute w-64 h-64 sm:w-96 sm:h-96 bg-gradient-to-r from-violet-500/5 to-indigo-500/5 rounded-full blur-3xl animate-float-reverse -z-10" style={{ bottom: '-10%', right: '-5%' }} />
        </div>
      </div>
      ...
      <section
        ref={composedRef}
        className={cn(
          'fixed inset-0 flex flex-col w-screen h-screen overflow-y-auto bg-gradient-to-br from-white via-purple-50/50 to-purple-100/50',
          'dark:bg-gradient-to-br dark:from-slate-900 dark:via-purple-800/20 dark:to-slate-900',
          disabled ? 'z-10' : 'z-20'
        )}
      >
        {/* Header Navigation */}
        <div className="fixed top-0 left-0 right-0 z-20 flex items-center justify-between px-4 xs:px-6 sm:px-8 md:px-10 lg:px-16 py-4 xs:py-5 sm:py-6 md:py-7 border-b border-black/10 dark:border-white/10 backdrop-blur-sm">
          <a 
          ...
          <div className="flex items-center gap-2 sm:gap-4">
            ...
            <Button
              ...
              disabled={disabled}
              className="bg-gradient-to-r from-purple-500 to-violet-500 hover:from-purple-600 hover:to-violet-600 text-white px-3 xs:px-4 sm:px-6 md:px-8 py-1.5 xs:py-2 sm:py-2.5 rounded-lg font-semibold text-xs sm:text-sm shadow-lg hover:shadow-2xl transition-all duration-300 transform hover:scale-105 border border-purple-400/30 flex-shrink-0 ml-2"
            >
              {startButtonText}
            </Button>
          </div>
        ...
        {/* Main Content */}
        <div className="relative z-10 flex-1 flex items-start w-full pt-[74px] xs:pt-[82px] sm:pt-[90px] md:pt-[98px] pb-[58px] xs:pb-[66px] sm:pb-[74px] md:pb-[82px]">
          <div className="w-full grid grid-cols-1 lg:grid-cols-2 gap-8 xs:gap-10 sm:gap-12 md:gap-14 lg:gap-16 px-6 xs:px-8 sm:px-10 md:px-12 lg:px-16 py-12 xs:py-14 sm:py-16 md:py-18 lg:py-20 auto-rows-max lg:auto-rows-fr h-full">
            
            {/* Left Column */}
            <div className="flex flex-col justify-center space-y-8 xs:space-y-10 sm:space-y-12 md:space-y-14 lg:space-y-16 order-1 lg:order-1 w-full">
              <div className="space-y-4 xs:space-y-5 sm:space-y-6 md:space-y-8 animate-in fade-in slide-in-from-bottom-8 duration-1000">
                <div>
                  <span className="inline-block px-2.5 xs:px-3 sm:px-4 py-1 xs:py-1.5 bg-gradient-to-r from-purple-500/10 to-violet-500/10 border border-violet-500/30 rounded-full text-xs font-semibold text-violet-600 dark:text-violet-300 tracking-widest uppercase">
                    Next Generation Voice AI
                  </span>
                </div>
                <h1 className="text-2xl xs:text-3xl sm:text-4xl md:text-4xl lg:text-5xl xl:text-6xl font-bold text-slate-700 dark:text-white leading-tight tracking-tight">
                  Conversational AI 
                  <span className="block bg-gradient-to-r from-purple-400 via-violet-400 to-indigo-400 bg-clip-text text-transparent">
                    Redefined
                  </span>
                </h1>
                <p className="text-sm xs:text-base sm:text-lg md:text-lg text-slate-600 dark:text-slate-300 leading-relaxed max-w-xl font-light">
                  Enterprise-grade voice technology with human-like natural language understanding. Deploy sophisticated AI conversations at scale with millisecond responsiveness.
                </p>
              </div>
              ...
              <div className="space-y-3 xs:space-y-4 sm:space-y-5">
                {features.map((feature, index) => (
                  <div
                    key={index}
                    className={`group p-3 xs:p-4 sm:p-5 md:p-6 rounded-xl bg-black/5 dark:bg-white/5 hover:bg-black/10 dark:hover:bg-white/10 border border-black/10 dark:border-white/10 hover:border-violet-500/30 transition-all duration-500 transform ${
                  ...
                  >
                    <div className="flex items-start gap-3 xs:gap-4">
                      <div className="w-9 h-9 xs:w-10 xs:h-10 rounded-lg bg-gradient-to-br from-purple-500/10 to-violet-500/10 border border-violet-500/30 flex items-center justify-center flex-shrink-0 text-violet-500 dark:text-violet-400 group-hover:text-violet-400 dark:group-hover:text-violet-300 transition-colors">
                        {getFeatureIcon(index)}
                      </div>
                      <div className="min-w-0">
                        <h3 className="font-semibold text-slate-700 dark:text-white text-sm sm:text-base mb-0.5 sm:mb-1 group-hover:text-violet-500 dark:group-hover:text-violet-300 transition-colors">
                          {feature.title}
                        </h3>
              ...
              <div className="pt-2 xs:pt-3 sm:pt-4">
                <Button
                  ...
                  disabled={disabled}
                  className="w-full lg:w-auto bg-gradient-to-r from-purple-600 to-violet-600 hover:from-purple-700 hover:to-violet-700 text-white px-4 xs:px-6 sm:px-8 md:px-10 py-2.5 xs:py-3 sm:py-3.5 md:py-4 rounded-lg font-semibold text-sm xs:text-base md:text-lg shadow-xl hover:shadow-2xl transition-all duration-300 transform hover:scale-105 border border-violet-400/40 hover:border-violet-300/60"
                >
                  Start Right Now
                </Button>
              </div>
            ...
            {/* Right Column */}
            <div className="flex flex-col justify-start items-center gap-26 order-1 lg:order-2 w-full">
              ...
              <div className="w-full">
                ...
                >
                  {/* Glowing backdrop */}
                  <div className="absolute inset-0 bg-gradient-to-br from-purple-600/30 via-violet-600/20 to-indigo-600/30 rounded-3xl blur-2xl opacity-60 group-hover:opacity-100 transition-opacity duration-500 animate-pulse-ring" />

                  {/* Glowing ring around avatar */}
                  <div className="absolute w-72 h-72 rounded-full border-2 border-violet-500/30 blur-lg animate-spin-slow" />

                  {/* Floating outer rings */}
                  <div className="absolute w-64 h-64 rounded-full border border-violet-500/20 animate-spin-slow reverse" />
                  <div className="absolute w-48 h-48 rounded-full border border-purple-500/20 animate-spin-slow" />

                  {/* Main Icon */}
                  <div 
                    className="relative w-28 h-28 xs:w-32 xs:h-32 sm:w-40 sm:h-40 md:w-48 md:h-48 lg:w-56 lg:h-56 bg-gradient-to-br from-purple-500 to-violet-500 rounded-3xl flex items-center justify-center shadow-2xl group-hover:shadow-violet-500/50 transition-all duration-500 transform group-hover:scale-110 group-hover:-rotate-3 flex-shrink-0"
                    style={{
                  ...
                  >
                    <svg
                      ...
                      fill="none"
                      xmlns="http://www.w3.org/2000/svg"
                      className="text-white drop-shadow-lg w-10 h-10 xs:w-12 xs:h-12 sm:w-16 sm:h-16 md:w-20 md:h-20"
                    >
                      <path
              ...
              {/* Stats Carousel */}
              <div className="w-full max-w-xs xs:max-w-sm rounded-2xl p-5 xs:p-6 sm:p-8 md:p-10 bg-gradient-to-br from-black/5 to-black/3 dark:from-white/5 dark:to-white/3 border border-black/10 dark:border-white/10 backdrop-blur-xl">
                <div className="text-center">
                  <div className="relative h-24 xs:h-28 sm:h-32 md:h-36 overflow-hidden">
                    {stats.map((stat, idx) => (
                      ...
                      >
                        <p className="text-3xl xs:text-4xl sm:text-5xl md:text-6xl font-bold bg-gradient-to-r from-purple-500 to-violet-500 bg-clip-text text-transparent mb-1.5 xs:mb-2 sm:mb-3 md:mb-4">
                          {stat.value}
                        </p>
                        <p className="text-slate-700 dark:text-white font-semibold text-xs xs:text-sm md:text-base">{stat.label}</p>                      
                        <p className="text-slate-500 dark:text-slate-400 text-xs mt-0.5">{stat.sublabel}</p>
                      </div>
                    ))}
                  </div>

                  <div className="flex justify-center gap-1.5 xs:gap-2 md:gap-3 mt-3 xs:mt-4 sm:mt-6 md:mt-8">
                    {stats.map((_, idx) => (
                      <button
                        key={idx}
                        ...
                        className={`rounded-full transition-all duration-500 ${
                          idx === currentStatSlide
                            ? 'w-8 h-2 xs:h-2.5 md:h-3 bg-gradient-to-r from-purple-400 to-violet-400' 
                            : 'w-1.5 xs:w-2 md:w-3 h-1.5 xs:h-2 md:h-3 bg-black/30 dark:bg-white/30 hover:bg-black/50 dark:hover:bg-white/50'
                        }`}
        ...
        {/* Footer */}
        <div className="relative bottom-0 left-0 right-0 z-20 border-t border-black/10 dark:border-white/10 px-3 xs:px-4 sm:px-6 md:px-8 lg:px-16 py-3 xs:py-4 sm:py-5 md:py-6">
          <p className="text-center text-xs sm:text-sm text-slate-600 dark:text-slate-400 font-light">
            Know more{' '}
            <a
              target="_blank"
              rel="noopener noreferrer"
              href="https://10xds.com/about-us/"
              className="text-violet-500 dark:text-violet-400 hover:text-violet-400 dark:hover:text-violet-300 font-semibold transition-colors duration-300 underline underline-offset-2"
            >
              about us
            </a>
